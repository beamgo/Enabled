...
<body>
         <form method="post" action="register-6.php" enctype="multipart/form-data">
         <?php    
    ...
    ?>
        ...
        echo $_SESSION["pid"];
                ...
                $namephoto = $_FILES["uppic2"]["name"];
        $tmp=explode('.',$namephoto);
        $extension=end($tmp);
                $tmpphoto = $_FILES["uppic2"]["tmp_name"];
                $icon = 'p-full';
                $space = '_';
                $dot = '.';
                $id = $_SESSION["pid"];
        ...
        mysqli_set_charset($conn, "utf8");
                ...
                if(copy($_FILES["uppic2"]["tmp_name"],"image/".$newname))
                {
                    echo "ย้ายรูปได้";
                }
                else 
